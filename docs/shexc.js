...
function hljsDefineShExC(hljs, opts = {}) {
  ...
  productions.IRIREF = {
    className: "symbol",
    begin: /</,
    end: />/, // can't use begin: IRIREF_RE because of contains.
  ...
  productions.base = {
    ...
    returnBegin: true,
    contains: [Object.assign({ endsParent: true }, productions.IRIREF)],
  };
  productions._import = {
    // Need a leading '_' because "import" is a js keyword.
  ...
  const shapeExprContentModel = [
         ...
         */
    {
      className: "regexp",
      begin: /\/([^\\/]|\\.)*\//,
      contains: [hljs.REGEXP_MODE],
    },

    productions.shape,
  ];
  const shapeExpression_keywords =
    "and or not closed abstract extends restricts iri bnode literal nonliteran" +
  ...
  productions.tripleExpression = {
    ...
    endsWithParent: true,
    keywords: shapeExpression_keywords,
    contains: [productions.IRIREF, productions.prefixedName].concat(
      shapeExprContentModel,
    ),
    relevance: 0,
  };
  ...
  if (!(startingProduction in productions))
    throw Error(
      `starting production ${startingProduction} not found in ${Object.keys(productions).join(", ")}}`,
    );
